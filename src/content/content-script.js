...
(function () {
  ...
  function matchesLanguageOption(optionNormalized, variation) {
    ...
    return (
      optionNormalized.includes(normalizedVariation) ||
  ...
  function collectFieldOptions(element) {
    ...
    if (element.tagName?.toLowerCase() === 'select') {
      rawOptions = Array.from(element.options || []).map(
        (option) => option.textContent || option.value || ''
      );
    } else if (element.list) {
      rawOptions = Array.from(element.list.options || []).map(
  ...
  function buildFieldMeta(element) {
    ...
    if (
      (fieldType === 'pronouns' || fieldType === 'education') &&
  ...
  async function initialize() {
    if (isBlockedDomain()) {
      console.log(
        'AI Context Assistant: disabled on',
